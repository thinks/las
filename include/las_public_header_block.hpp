//=========================================================================
// Copyright (C) 2006-2008 by Tommy Hinks						  
// tommy[dot]hinks[at]gmail[dot]com  												  
//
// Contributors: 
//             1) Tommy Hinks <tommy[dot]hinks[at]gmail[dot]com> 2007-2008
//=========================================================================

#ifndef SALSA_LAS_PUBLIC_HEADER_BLOCK_HPP_INCLUDED
#define SALSA_LAS_PUBLIC_HEADER_BLOCK_HPP_INCLUDED
...
public:
	...
	static 
	thx::int64 
	point_data_record_lengths(const thx::uint8 id)
	{
		switch(id)
		{
		case 0:
		...
		case 1:
			return 28;
		default:
			SALSA_THROW("LAS PHB: invalid id: " << static_cast<thx::int64>(id));
		}
	}
...
#pragma pack(1)		// Alignment 1 byte
	...
	public:
		...
		static const thx::int64 size = 227;	// [bytes]
	...
	public:
		...
		header()
			: reserved     (0),
			  ...
			  header_size(static_cast<thx::uint16>(header::size)),
			  offset_to_data(
				  static_cast<thx::uint16>(header::size + sizeof(thx::uint16))),	
			  num_variable_length_records(0),
			  point_data_format_id(0), 
			  point_data_record_length(	// NB: Only ID supplied.
				  static_cast<thx::uint16>(
				      point_data_record_lengths(point_data_format_id))),	
			  num_point_records(0),
			  x_scale_factor   (1.0), 
			  y_scale_factor   (1.0), 
			  z_scale_factor   (1.0),
		...
		{	
			...
			//
			memset(guid_data_4   , 0, guid_data_4_length*sizeof(thx::uint8));
			memset(sys_identifier, 0, sys_identifier_length*sizeof(thx::int8));
			memset(gen_software  , 0, gen_software_length*sizeof(thx::int8));
			memset(num_points_by_return, 0, 
				   num_points_by_return_length*sizeof(thx::int32));
		}

		header(const header& rhs) { *this = rhs; }

		~header() { SALSA_STATIC_ASSERT(size == sizeof(header), invalid_size); }

		header& 
		operator=(const header& rhs)
		{	// Bit-wise copy.
			//
			memcpy(this, &rhs, header::size);
			return *this;
		}
...
public:
	...
	static const thx::uint16 pre_point_value = 52445;	// 0xCCDD = 52445.

public:

	las_public_header_block()
		: hdr_(),
		  pre_point_data_(las_public_header_block::pre_point_value)
	{}

	explicit
	las_public_header_block(const header& hdr)
		: hdr_(hdr),
		  pre_point_data_(las_public_header_block::pre_point_value)
	{}

	explicit
	las_public_header_block(const std::string&              sys_id,
...
operator<<(basic_ostream<CharT,Traits>& os, 
		   const salsa::las_public_header_block& rhs)
{
	os	<< "salsa::las_public_header_block[0x" << &rhs << "]:\n"
		<< "File Signature ('LASF')           : '" 
		<< std::string(rhs.file_signature()) << "'\n"
		<< "Reserved                          : " 
			<< static_cast<thx::int64>(rhs.reserved()) << "\n"
		<< "GUID data 1                       : " 
			<< static_cast<thx::int64>(rhs.guid_data_1()) << "\n"
		<< "GUID data 2                       : " 
			<< static_cast<thx::int64>(rhs.guid_data_2()) << "\n"
		<< "GUID data 3                       : " 
			<< static_cast<thx::int64>(rhs.guid_data_3()) << "\n"
