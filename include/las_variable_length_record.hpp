...
public:
    ...
    //! Copy CTOR.
    variable_length_record(const variable_length_record &rhs) 
    { *this = rhs; }


    //! Assign.
    variable_length_record& 
    operator=(const variable_length_record &rhs)
    {
    ...
    { 
        ...
        LAS_STATIC_ASSERT(size == sizeof(variable_length_record), 
                          invalid_vlr_size); 
    }
           ...
           const las::variable_length_record &rhs)
{
    os	<< "las::variable_length_record[0x" << &rhs << "]\n"
        << "sizeof(las::variable_length_record)  : " 
            << sizeof(las::variable_length_record) << "\n"
        << "Record Signature                     : " 
            << std::hex 
            << "0x" << rhs.record_signature() << " | Valid: 0x" 
            <<  las::variable_length_record::valid_record_signature << "\n"
            << std::dec
        << "User Id                              : '" 
